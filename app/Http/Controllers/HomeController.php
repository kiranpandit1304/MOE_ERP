...
class HomeController extends Controller
{
        ...
        if ($shop != null && $type != null) {
            ...
            if ($type == 'all-products') {
                ...
                if ($min_price != null && $max_price != null) {
                    $products->where(DB::raw('(unit_price - discount)'), '>=', $min_price)->where(DB::raw('(unit_price - discount)'), '<=', $max_price);
    ...
    public function all_seller(Request $request)
    {
        ...
        if($district_id != null)
        {
            $shops = $shops->where('district_id', $district_id);
        }elseif($city_id != null)
        {
        ...
        $query='';
        return view('frontend.shop_listing', compact('shops','query'));
    }

    public function sellerListingByCategory(Request $request, $category_slug)
    {
        $category = Category::where('slug', $category_slug)->first();
        $category_id = $category->id;
        //get all shops has products in this category
        
        $category_ids = CategoryUtility::children_ids($category_id);
        $category_ids[] = $category_id;

        
        $shops = Shop::where('verification_status', 1)->whereHas('products', function($q) use($category_ids){
        ...
        $query='';
        return view('frontend.shop_listing', compact('shops','query','category_id'));
        abort(404);
    }
