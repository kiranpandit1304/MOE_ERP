...
class AddressController extends Controller
{
    ...
    {
        return new DistrictsCollection(District::where('status', 1)->get());
    }
    ...
    public function getDistrictsByCity($city_id,Request $request)
    {
        $district_query = District::where('status', 1)->where('city_id',$city_id);
        if ($request->name != "" || $request->name != null) {
             $district_query->where('name', 'like', '%' . $request->name . '%');
        }
        $districts = $district_query->get();
        return new DistrictsCollection($districts);
    }
