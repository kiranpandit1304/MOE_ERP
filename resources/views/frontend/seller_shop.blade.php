                    ...
                    <div class="col pl-5 pl-md-0 ml-5 ml-md-0">
                        <div class="d-lg-flex align-items-center justify-content-lg-end">
                            <div class="d-md-flex justify-content-md-end align-items-md-baseline">
                                <!-- Member Since -->
                                <div class="px-md-3 mt-2 mt-md-0 border-md-right">
                                    <div class="fs-12 fw-400 text-secondary">{{ $shop->address }}</div>
                                    <div class="mt-1 fs-12 fw-700 text-secondary">{{ $shop->city->name }}@if($shop->district), {{ $shop->district->name }}  @endif</div>
                                </div>
                                @if($shop->phone || $shop->mobile || $shop->whatsapp)
                                <div class="px-md-3 mt-2 mt-md-0 border-md-right">
                                    @if($shop->phone) <div class="text-secondary"><small>{{ translate('Phone') }}</small> : {{$shop->phone}}</div> @endif
                                    @if($shop->mobile) <div class="text-secondary"><small>{{ translate('Mobile') }} </small> : {{$shop->mobile}}</div> @endif
                                    @if($shop->whatsapp) <div class="text-secondary"><small>{{ translate('WhatsApp') }} </small> : {{$shop->whatsapp}}</div> @endif
                                </div>
                                @endif
                            ...
                            <div class="d-flex justify-content-md-end pl-lg-3 pt-3 pt-lg-0">
                                ...
                                @if ($shop->facebook || $shop->instagram || $shop->google || $shop->twitter || $shop->youtube)
                                    <div class="pl-md-3 pr-lg-3 mt-2 mt-md-0">
                                        <ul class="social-md colored-light list-inline mb-0 mt-1">
                                            ...
                                            @if ($shop->website)
                                            <li class="list-inline-item">
                                                <a href="{{ $shop->website }}" class="website"
                                                    target="_blank">
                                                    <i class="lab la-link"></i>
                                                </a>
                                            </li>
                                            @endif
                                        </ul>
                                        ...
                                        <div class="p-3 mr-3">
                                            <div class="aiz-range-slider">
                                                <div
                                                    ...
                                                    data-range-value-min="@if(\App\Models\Product::where(['user_id' => $shop->user->id, 'published' => 1, 'approved' => 1])->count() < 1) 0 @else {{ \App\Models\Product::where(['user_id' => $shop->user->id, 'published' => 1, 'approved' => 1])->min(DB::raw('(unit_price - discount)')) }} @endif"
                                                    data-range-value-max="@if(\App\Models\Product::where(['user_id' => $shop->user->id, 'published' => 1, 'approved' => 1])->count() < 1) 0 @else {{ \App\Models\Product::where(['user_id' => $shop->user->id, 'published' => 1, 'approved' => 1])->max(DB::raw('(unit_price - discount)')) }} @endif"
                                                ></div>
                                                ...
                                                <div class="row mt-2">
                                                    <div class="col-6">
                                                        <span class="range-slider-value value-low fs-14 fw-600 opacity-70"
                                                            ...
                                                            @elseif($products->min(DB::raw('(unit_price - discount)')) > 0)
                                                                data-range-value-low="{{ $products->min(DB::raw('(unit_price - discount)')) }}"
                                                            @else
                                                    ...
                                                    <div class="col-6 text-right">
                                                        <span class="range-slider-value value-high fs-14 fw-600 opacity-70"
                                                            ...
                                                            @elseif($products->max(DB::raw('(unit_price - discount)')) > 0)
                                                                data-range-value-high="{{ $products->max(DB::raw('(unit_price - discount)')) }}"
                                                            @else
